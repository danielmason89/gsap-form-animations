...
containers.forEach((container) => {
  ...
  input.addEventListener("focus", () => {
    // Check to see if any text in input
    if (!input.value) {
      tl.fromTo(
        line,
        { attr: { d: start } },
        { attr: { d: end }, ease: "Power2.easOut", duration: 0.75 }
      );
      tl.to(
        line,
        {
...
form.addEventListener("click", () => {
  containers.forEach((container) => {
    ...
    // Validating email and phone here
    // Name Validation
...
// checking email validation and phone number validation
...
// Colorize input function

function colorize(color, line, placeholder) {
  gsap.to(line, { stroke: color, duration: 0.75 });
  gsap.to(placeholder, { stroke: color, duration: 0.74 });
}
...
gsap.fromTo(
  "#eyebrow",
  { y: 0 },
  { y: -3, repeat: -1, yoyo: true, repeatDelay: 0.5, ease: "Power2.easeOut" }
);
